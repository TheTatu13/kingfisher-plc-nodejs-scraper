...
|---|---|
| `KINGFISHER INFORMATION TECHNOLOGY SERVICES (ROMANIA) S.R.L.` | legal name, uppercase (e.g. `EXAMPLE COMPANY SRL`) |
| `Kingfisher` | commercial brand |
| `51987250` | fiscal code (CUI), no `RO` prefix |
| `https://www.kingfisher.com` | `https://www.example.com` |
| `https://careers.kingfisher.com/job-search` | the open-positions listing page |
| `` | the job sitemap URL (or `""` if the site has none) |
| `https://www.kingfisher.com/jobs/` | canonical job-permalink prefix, e.g. `https://www.example.com/jobs/` |
...
| `TheTatu13` / `kingfisher-plc-nodejs-scraper` | the derived repo's owner / name |
